...
class CoverScraper:
    ...
    async def fetch_page(self, url: str) -> str | None:
        ...
        try:
            ...
            # YEN PRESS hydration
                ...
                try:
                    await page.wait_for_selector(
                        "img[src*='cover'], div[class*='BookDetail'], div.content-heading-txt, div.buy-from",
                        timeout=3000
                    )
                except:
                    pass
            ...
            # HANASHI MEDIA hydration
                ...
                try:
                    # 1. Target the exact elements from the inspected DOM
                    # Wait for BOTH the image and the story div to land in the DOM
                    await page.wait_for_selector("img[src*='store-api.hanashi.media/ebooks/cover']", timeout=7000)
                    await page.wait_for_selector("div.whitespace-pre-wrap", timeout=7000)
                    
                    # 2. Give SvelteKit a short buffer (500ms - 1000ms) to finish reactivity/binding
                    await page.wait_for_timeout(750)
                except Exception as e:
                    # Fallback log to see if it timed out waiting for dynamic JS content
                    print(f"⚠️ Hanashi Media hydration wait timed out or failed: {e}")
    ...
    async def get_cover(self, url: str, title: str = None, volume: str = None) -> str | None:
        ...
        if "sevenseasentertainment.com" in url:
            data = self.seven.get_cover(url)  # Returns dict: {"cover_url", "synopsis", "vendors"}
            if data and data.get("cover_url"):
                print(f"✔ Seven Seas cover & metadata fetched for {title} vol {volume}")
                return data
            elif data:
                # In case image failed/missing, but synopsis/vendors were extracted
                print(f"⚠️ Seven Seas metadata fetched without image for {title} vol {volume}")
                return data

            print(f"❌ Seven Seas failed to scrape {title} vol {volume}")
            return None
        ...
        if "yenpress.com" in url:
            yen_data = self.yen.parse(html)
            img_url = yen_data.get("cover_url")
            synopsis = yen_data.get("synopsis")
            vendors = yen_data.get("vendors")

            # Validate image if present
            valid_cover_url = None
            if img_url:
                try:
                    resp = requests.get(img_url, timeout=10)
                    if resp.status_code == 200 and not is_placeholder_image(resp.content):
                        valid_cover_url = img_url
                    else:
                        print("❌ Yen Press placeholder detected")
                except Exception as e:
                    print(f"❌ Error downloading Yen Press image: {e}")

            # Always log status accurately
            if valid_cover_url or synopsis:
                print(f"✔ Yen Press metadata fetched for {title} vol {volume}")

            # ALWAYS return structured payload as long as we parsed the page
            return {
        ...
        if "j-novel.club" in url and volume:
            result = await self.jnovel.parse(html, url=url, volume=volume)
            if result and result.get("cover_url"):
                print(f"✔ J-Novel Club cover & metadata saved for {title} vol {volume}")
                return result

        # 4. Cross Infinite World
        if "crossinfworld.com" in url:
            result = self.crossinf.parse(html, url=url)
            if result and result.get("cover_url"):
                print(f"✔ Cross Infinite World metadata saved for {title} vol {volume}")
                return result

        # 5. Square Enix
        if "squareenixmangaandbooks.square-enix-games.com" in url:
            result = await self.squareenix.parse(html, url=url)
            if result and result.get("cover_url"):
                print(f"✔ Square Enix metadata saved for {title} vol {volume}")
                return result

        # 6. Hanashi Media
        if "hanashi.media" in url:
            # Pass base_url instead of url
            data = self.hanashimedia.parse(html, base_url=url)
            
            if isinstance(data, dict):
                cover = data.get("cover_url")
                synopsis = data.get("synopsis")

                if cover or synopsis:
                    print(f"✔ Hanashi Media cover & metadata saved for {title} vol {volume}")
                    return data

            print(f"⚠️ Hanashi Media scraper completed, but no cover/synopsis found for {title} vol {volume}")
